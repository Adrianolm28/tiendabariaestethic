...
(function(window, $, undefined) {
     ...
     * ====================================================================== */    
         ...
         * ====================================================================== */    
            ...
            popup_input.on('click', '.fa', function() {
                var $this = $(this);
                var number = $this.closest('.wcs_popup_input').attr('data-number');
                var message = $this.siblings('input').val();
    
                go_to_whatsapp(number, message);
            });     
         ...
         * ====================================================================== */     
            ...
            function go_to_whatsapp(number, text) {
                ...
                var url = WhatsAppUrl + '?phone=' + number + '&text=' + encodeURIComponent(text);
                console.log(url);
         ...
         * ====================================================================== */         
            ...
            debug.append("<div><strong>Original Date</strong> " + now.format('YYYY-MM-DD HH:mm:ss') + " <br><strong>UTC Offset</strong> " + (now.utcOffset()/60) + "</div>");
    
            if(settings.timezone != '' && settings.now == '') { // set timezone
                now.tz(settings.timezone);
                debug.append("<div><strong>Setting Timezone</strong> " + now.format('YYYY-MM-DD HH:mm:ss') + " <br><strong>UTC Offset</strong> " + (now.utcOffset()/60) + "</div>");
            }
    
            if(settings.now != '') {
                now = moment(settings.now, 'YYYY-MM-DD HH:mm:ss');
                debug.append("<div><strong>Setting Manual Date</strong> " + now.format('YYYY-MM-DD HH:mm:ss') + " <br><strong>UTC Offset</strong> " + (now.utcOffset()/60) + "</div>");
            }
    ...
    function sendMessage(icon) {
        var inputDiv = icon.closest('.wcs_popup_input');
        var number = inputDiv.getAttribute('data-number');
        var message = inputDiv.querySelector('input').value;
