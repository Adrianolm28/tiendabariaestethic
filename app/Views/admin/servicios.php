                        ...
                        <div class="modal fade" id="myModal">
                            <div class="modal-dialog modal-lg">
                                <div class="modal-content">
                                    ...
                                    <form method="post" action="<?php echo site_url('admin/servicios/store'); ?>" id="addServicios" name="addServicios" enctype="multipart/form-data">
                                        <input type="hidden" id="id" name="id">

                                        <div class="card-body">
                                            <div class="form-group">
                                                <label for="exampleInputEmail1">titulo</label>
                                                <input type="text" class="form-control" name="txtTitulo" id="txtTitulo" placeholder="">
                                            </div>

                                            <div class="form-group">
                                                <label for="exampleInputEmail1">Descripcion</label>
                                                <input type="text" class="form-control" name="txtDescripcion" id="txtDescripcion" placeholder="">
                                            </div>
                                            ...
                                            <div class="fileupload fileupload-new" data-provides="fileupload">
                                                ...
                                                <div>
                                                    ...
                                                    <label class="btn btn-primary btn-file">
                                                        ...
                                                        <input type="file" accept=".jpg, .jpeg, .png" name="icono" id="icono" style="display: none;">
                                                    </label>
                        ...
                        <table id="serviciosTable" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                <?php foreach ($servicios as $item) : ?>
                                    <tr>
                                        <td><?= $item['id']; ?></td>
                                        <td><img src="<?= base_url('public/assets/image/others/' . $item['icono']); ?>" style="width:40px; height:40px;"></td>
                                        <td><?= $item['titulo']; ?></td>
...
<script>
    $(document).ready(function() {
        ...
        $('#addServicios').on('submit', function(event) {
        ...
        $('body').on('click', '.btnEdit', function() {
            ...
            $.ajax({
                url: '<?= site_url('admin/servicios/edit/') ?>' + id, // Ajusta la URL según tu estructura de rutas
                type: 'GET',
                dataType: 'json',
                success: function(res) {
                    if (res.status) {
                        ...
                        $('#myModal').modal('show');
                        $(' #id').val(res.data.id);
                        $(' #txtTitulo').val(res.data.titulo);
                        $(' #txtDescripcion').val(res.data.descripcion);
                        if (res.data.icono) {
                            $('#lim').html('<img src="<?= base_url('public/assets/image/others/'); ?>' + res.data.icono + '" style="max-width: 200px; max-height: 150px;">');
                        } else {
        ...
        $('body').on('click', '.btnDelete', function() {
            ...
            Swal.fire({
                ...
                $.ajax({
                    url: '<?= site_url('admin/servicios/actualizar_estado/') ?>' + id + '/' + newStatus,
                    type: 'POST',
                    dataType: 'json',
                    success: function(res) {
                        location.reload();
