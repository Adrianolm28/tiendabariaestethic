            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Anuncio Principal</h3>
                        ...
                        <div class="modal fade" id="myModal">
                            <div class="modal-dialog modal-lg">
                                <div class="modal-content">
                                    ...
                                    <form method="post" action="<?php echo site_url('admin/anunciotienda/store'); ?>"
                                        id="addanunciotienda" name="addBannertienda" enctype="multipart/form-data">
                                                ...
                                                <div>
                                                    ...
                                                    <label class="btn btn-primary btn-file">
                                                        ...
                                                        <input type="file" accept=".jpg, .jpeg, .png"
                                                            name="imagenanuncio" id="imagenanuncio"
                                                            style="display: none;">
                                                    </label>
                    ...
                    <!-- /.card-header -->
                    <div class="card-body">
                        <table id="anunciotiendaTable" class="table table-bordered table-striped">
                            <thead>
                                <tr>
                                    <th>Id</th>
                                    <th>Imagen</th>
                            ...
                            <tbody>
                                <?php foreach ($anunciotienda as $item) : ?>
                                    <tr>
                                        <td><?= $item['id']; ?></td>
                                        <td><img src="<?= base_url('public/assets/tienda/img/' . $item['imagen_anuncio']); ?>" style="width:40px; height:40px;"></td>
...
<script>
    $(document).ready(function() {
        $('#anuciotiendaTable').DataTable();

        $('#addanunciotienda').on('submit', function(event) {
        ...
        $('body').on('click', '.btnEdit', function() {
            ...
            $.ajax({
                url: '<?= site_url('admin/anunciotienda/edit/') ?>' +
                ...
                success: function(res) {
                    console.log('respuesta', res)
                     if (res.status) {
                        ...
                        $('#myModal').modal('show');
                        $(' #id').val(res.data.id);
                        $('#id_categorias').val(res.data.id_categoria);
                        $('#id_producto').val(res.data.id_producto);
                      

                        if (res.data.imagen_anuncio) {

                            $('#lim').html(
                                '<img src="<?= base_url('public/assets/tienda/img/'); ?>' +
                                res.data.imagen_anuncio +
        ...
        $('body').on('click', '.btnDelete', function() {
            ...
            Swal.fire({
                ...
                $.ajax({
                    url: '<?= site_url('admin/anunciotienda/actualizar_estado/') ?>' + id +
                        '/' + newStatus,
                    type: 'POST',
                    dataType: 'json',
                    success: function(res) {
