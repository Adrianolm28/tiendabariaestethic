<section class="content">
   <!--  <?php print_r($respaldo); ?> -->
    <div class="container-fluid">
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">serivios</h3>
                            ...
                            <div class="modal-dialog modal-lg">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h4 class="modal-title">Agregar Nuena imagen</h4>
                                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                                    </div>

                                    <div id="messageContainer"></div>

                                    <form method="post" action="<?php echo site_url('admin/respaldo/store'); ?>" id="addRespaldo" name="addRespaldo" enctype="multipart/form-data">
                                        ...
                                        <div class="card-body">
                                            ...
                                            <div class="fileupload fileupload-new" data-provides="fileupload">
                                                ...
                                                <div class="fileupload-preview fileupload-exists thumbnail" id="lim" style="max-width: 200px; background-color: black; max-height: 150px; line-height: 20px;"></div>
                                                <div>
                                                    ...
                                                    <label class="btn btn-primary btn-file">
                                                        ...
                                                        <input type="file" accept=".jpg, .jpeg, .png" name="logo_respaldo" id="logo_respaldo" style="display: none;">
                                                    </label>
                    ...
                    <!-- /.card-header -->
                    <div class="card-body">
                        <table id="respaldoTable" class="table table-bordered table-striped ">
                            <thead>
                                <tr>
                                    <th>id</th>
                                    <th >imagen</th>
                            ...
                            <tbody>
                                <?php foreach ($respaldo as $item) : ?>
                                    <tr>
                                        <td><?= $item['id']; ?></td>
                                        <td style="text-align: center;" class="table-success"><img src="<?= base_url('public/assets/image/others/' . $item['logo_respaldo']); ?>" style="height:40px;"></td>
                                        <td>
...
<script>
    $(document).ready(function() {
        $('#respaldoTable').DataTable();

        $('#addRespaldo').on('submit', function(event) {
        ...
        $('body').on('click', '.btnEdit', function() {
            ...
            $.ajax({
                url: '<?= site_url('admin/respaldo/edit/') ?>' + id, // Ajusta la URL según tu estructura de rutas
                type: 'GET',
                dataType: 'json',
                success: function(res) {
                    if (res.status) {
                        ...
                        $('#myModal').modal('show');
                        $(' #id').val(res.data.id);
                        if (res.data.logo_respaldo) {
                            $('#lim').html('<img src="<?= base_url('public/assets/image/others/'); ?>' + res.data.logo_respaldo + '" style="max-width: 200px; max-height: 150px;">');
                        } else {
        ...
        $('body').on('click', '.btnDelete', function() {
            ...
            Swal.fire({
                ...
                $.ajax({
                    url: '<?= site_url('admin/respaldo/actualizar_estado/') ?>' + id + '/' + newStatus,
                    type: 'POST',
                    dataType: 'json',
                    success: function(res) {
                        location.reload();
