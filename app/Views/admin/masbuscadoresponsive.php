                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                            <button id="btnAgregarMasBuscado" class="btn btn-primary" type="button">
                            ...
                            <div class="modal-dialog modal-lg">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h4 class="modal-title">Agregar Nuevo Banner de Promoción</h4>
                                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                                    </div>
                                    <div id="messageContainerAgregar"></div>
                                    <form method="post" action="<?php echo site_url('admin/masbuscadoresponsive/store'); ?>" id="addMasBuscado" name="addMasBuscado" enctype="multipart/form-data">
                        ...
                        <div class="modal fade" id="myModalEditar">
                            <div class="modal-dialog modal-lg">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h4 class="modal-title">Editar Banner de Promoción</h4>
                                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                                    </div>
                                    <div id="messageContainerEditar"></div>
                                    <form method="post" action="<?php echo site_url('admin/masbuscadoresponsive/store'); ?>" id="editMasBuscado" name="editMasBuscado" enctype="multipart/form-data">
                        ...
                        <table id="bannertiendaTable" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                <?php foreach ($banners as $item) : ?>
                                    <tr>
                                        <td><?= $item['id']; ?></td>
                                        <td><img src="<?= base_url('public/assets/image/img_tienda/masbuscadoresponsive/' . $item['imagenbanner']); ?>" style="width:150px; height:90px;"></td>
                                        <td><?= $item['id_categorias'] ?></td>
                                        <td><?= $item['estado'] == 1 ?  '<span class="me-1 badge bg-success">Activo</span>' : '<span class="me-1 badge bg-danger">Inactivo</span>'; ?></td>
                                        <td>
...
$(document).ready(function() {
    ...
    $('#btnAgregarMasBuscado').on('click', function() {
        $('#myModalAgregar').modal('show');
        $('#addMasBuscado')[0].reset();
        $('#lim').html('');
    });

    // Guardar nuevo banner
    $('#addMasBuscado').on('submit', function(event) {
    ...
    $('body').on('click', '.btnEdit', function() {
        ...
        $.ajax({
            url: '<?= site_url('admin/masbuscadoresponsive/edit/') ?>' + id,
            type: 'GET',
            dataType: 'json',
            success: function(res) {
                if (res.status) {
                    ...
                    if (res.data.imagenbanner) {
                        $('#edit_lim').html('<img src="<?= base_url('public/assets/image/img_tienda/masbuscadoresponsive/'); ?>' + res.data.imagenbanner + '" style="max-width: 200px; max-height: 150px;">');
                    } else {
    ...
    // Guardar edición
    $('#editMasBuscado').on('submit', function(event) {
        event.preventDefault();
        var form_data = new FormData(this);
        var form_action = $(this).attr('action');
        $.ajax({
    ...
    // Al editar, cargar datos en los campos nuevos
    $('body').on('click', '.btnEdit', function() {
        var id = $(this).attr('data-id');
        $.ajax({
            url: '<?= site_url('admin/masbuscadoresponsive/edit/') ?>' + id,
    ...
    $('body').on('click', '.btnDelete', function() {
        ...
        Swal.fire({
            ...
            $.ajax({
                url: '<?= site_url('admin/masbuscadoresponsive/actualizar_estado/') ?>' + id + '/' + newStatus,
                type: 'POST',
                dataType: 'json',
                success: function(res) {
                     location.reload(); 
        ...
        $('body').on('click', '.btnEliminarRegistro', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '<?= site_url('admin/masbuscadoresponsive/eliminar/') ?>' + id, // Usa la URL correcta
