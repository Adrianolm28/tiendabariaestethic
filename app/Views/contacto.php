<?php
include('plantilla/header.php');
?>
<!-- offcanvas -->
<aside id="offcanvas-nav">
  <nav class="m-nav">
    <button id="nav-cls-btn"><i class="fa-solid fa-xmark"></i></button>
    <div class="logo">
      <a href="">
        <img src="./assets/image/others/logo/Logo.png" alt="logo-thumb" />
      </a>
    </div>
    <ul class="nav-links">
      <li class="dropdown"><a href="index.html">Home</a>

        <ul class="d-menu">
          <li><a href="home-page-02.html">Designer</a></li>
          <li><a href="home-page-03.html">Development</a></li>
          <li><a href="index-lawyer.html">Lawyer</a></li>
          <li><a href="index.html">Product Designer</a></li>
          <li><a href="Home-photographer.html">Photographer</a></li>
          <li><a href="Home-Teachers.html">Teachers</a></li>
        </ul>

      </li>
      <li class="dropdown"><a href="portfolio.html">Portfolio </a>

        <ul class="d-menu">
          <li><a href="portfolio.html">Portfolio</a></li>
          <li><a href="protfolio-detalis.html">protfolio-detalis</a></li>
        </ul>

      </li>
      <li class="dropdown">
        <a href="home-one-service.html">service</a>

        <ul class="d-menu">
          <li><a href="home-one-service.html">service</a></li>
          <li><a href="home-one-service-detalis.html">service-detalis</a></li>
        </ul>

      </li>
      <li class="dropdown"><a href="home-1-blog.html">Blog</a>

        <ul class="d-menu">
          <li><a href="home-1-blog.html">Blog</a></li>
          <li><a href="home-1-blog-2.html">Blog-02</a></li>
          <li><a href="home-1blog-detalis.html">Blog-detalis</a></li>
        </ul>

      </li>
      <li class="dropdown"><a href="shop.html">pages</a>


        <ul class="d-menu">

          <li>
            <a href="achievement.html">achievement</a>
          </li>
          <li>
            <a href="countact-me.html">countact-me</a>
          </li>
          <li>
            <a href="coming-soon.html">coming-soon</a>
          </li>
          <li>
            <a href="error.html"> 404 error</a>
          </li>
          <li>
            <a href="gallery.html"> gallery</a>
          </li>
          <li>
            <a href="product.html"> Product</a>
          </li>
          <li>
            <a href="product-details.html"> Product-details</a>
          </li>
          <li>
            <a href="team.html"> Team-Member</a>
          </li>
          <li>
            <a href="team-detalis.html"> Team-Details</a>
          </li>

        </ul>

      </li>

      <li class="dropdown">
        <a href="<?php echo base_url('/contacto'); ?>">Contactanos</a>
      </li>

    </ul>
    <ul class="social-icons">
      <li>
        <a href="#"><i class="fa-brands fa-facebook-f"></i></a>
      </li>
      <li>
        <a href="#"><i class="fa-brands fa-twitter"></i></a>
      </li>
      <li>
        <a href="#"><i class="fa-brands fa-youtube"></i></a>
      </li>
      <li>
        <a href="#"><i class="fa-brands fa-linkedin-in"></i></a>
      </li>
    </ul>
  </nav>
</aside>


<!-- offcanvas end -->














<main>


  <!-- .inner-bg-part start  -->


  <section class="inner-bg">
    <div class="container">
      <div class="row">
        <div class="col-lg-12">
          <div class="inner-bg-head">
          </div>
          <div class="inner-item">
            <div class="inner-text">
            </div>
            <div class="icon">
            </div>
            <div class="inner-text">
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>






  <!-- .inner-bg-part end  -->




  <!-- Contact Me part start  -->



  <section class="contact-me">
    <div class="container">
      <div class="row">
        <div class="col-lg-12">

          <div class="contuict-me-main">
            <div class="contact-me-item">
              <div class="contact-me-inner">
                <div class="icon">
                  <span>
                    <svg width="76" height="68" viewBox="0 0 76 68" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                      <mask id="mask0_215_1355" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="7" y="0" width="69" height="68">
                        <rect x="7.04688" width="67.9557" height="67.9557" fill="url(#pattern0)" />
                      </mask>
                      <g mask="url(#mask0_215_1355)">
                        <rect x="8.80469" y="-5.27148" width="68.5415" height="73.2281" fill="#2662FA" />
                      </g>
                      <circle cx="15" cy="15" r="15" fill="#DAED1A" />
                      <path d="M8.38281 15.8816L14.1181 20.2933L22.0593 9.70508" stroke="white" stroke-width="2" stroke-linecap="round" />
                      <defs>
                        <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                          <use xlink:href="#image0_215_1355" transform="scale(0.00195312)" />
                        </pattern>
                        <image id="image0_215_1355" width="512" height="512" xlink:href="data:image/png;base64," />
                      </defs>
                    </svg>

                  </span>
                </div>
                <div class="text">
                  <h3>Nuestra Dirección</h3>
                  <p>Calle Manrique de Luna Nro 168<br>
                    SAN MIGUEL</p>
                </div>
              </div>
            </div>
            <div class="contact-me-item">
              <div class="contact-me-inner">
                <div class="icon">
                  <span>
                    <!-- <svg width="82" height="66" viewBox="0 0 82 66" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M59.6871 14.9219H5C2.23858 14.9219 0 17.1605 0 19.9219V66.0003L14.9218 58.5394H59.6871V14.9219Z" fill="#DAED1A" />
                      <path d="M25.6602 0H81.4951V53.3741L64.8516 43.9224H20.6602V5C20.6602 2.23858 22.8987 0 25.6602 0Z" fill="#2662FA" />
                      <rect opacity="0.7" x="5.73828" y="0.574219" width="8.50744" height="8.03481" fill="#FF7236" />
                    </svg> -->

                    <img style="width: 92px;" src="./assets/image/others/contacto/wsp.png" alt="logo-thumb" />


                  </span>
                </div>
                <div class="text">
                  <h3>Contacto de informacion</h3>
                  <p>Llamenos o escribenos!
                  </p>
                  <h5>+51 933 696 269</h5>
                </div>
              </div>
            </div>
          </div>

        </div>
      </div>
    </div>
  </section>







  <!-- Contact Me part end  -->













  <!-- Thought part start  -->
  <section class="thought thought-3 ">
    <div class="container">
      <div class="row ">
        <div class="col-lg-5 p-r">
          <img style="width: 100%;height: 100%;" src="assets/image/contacto.png" alt="">
        </div>


        <div class="col-lg-7">
          <form id="formulario-contacto" method="post" action="<?= site_url('/enviar-correo') ?>" enctype="multipart/form-data"> <!-- No necesitas method="POST" dos veces -->
            <?php if (session()->has('success')) : ?>
              <div class="alert alert-success">
                <?= session('success') ?>
              </div>
            <?php endif; ?>
            <div id="form-messages"></div> <!-- Contenedor para mensajes de éxito/error -->
            <div class="send">
              <div class="send-item">
                <div class="send-inner">
                  <div class="mb-3">
                    <label for="tipo_documento" class="form-label">Tipo de Documento</label>
                    <select class="form-control" id="tipo_documento" name="tipo_documento">
                      <option value="ruc" selected>RUC</option>
                      <option value="dni">DNI</option>
                    </select>
                  </div>
                  <div class="mb-3">
                    <label for="numero_documento" class="form-label">Número de Documento</label>
                    <input type="text" class="form-control" id="numero_documento" name="numero_documento" placeholder="Ingrese número">
                  </div>
                </div>
                <div class="send-inner">
                  <div class="mb-3">
                    <label for="nombres" class="form-label">Nombres</label>
                    <input type="text" class="form-control" id="nombres" name="nombres" placeholder="Nombres completos">
                  </div>
                  <div class="mb-3">
                    <label for="email" class="form-label">E-mail</label>
                    <input type="email" class="form-control" id="email" name="email" placeholder="ejemplo@gmail.com">
                  </div>
                </div>
                <div class="send-inner">
                  <div class="mb-3">
                    <label for="telefono" class="form-label">Teléfono</label>
                    <input type="text" class="form-control" id="telefono" name="telefono" placeholder="+51 999999999">
                  </div>
                  <div class="mb-3">
                    <!-- Espacio vacío si quieres mantener la alineación, o puedes agregar otro campo aquí -->
                  </div>
                </div>
                <div class="mb-3">
                  <label for="descripcion" class="form-label">Descripción</label>
                  <textarea class="form-control" id="descripcion" name="descripcion" placeholder="Escribe tu consulta..." rows="4"></textarea>
                </div>
                <div class="send-btn">
                  <!-- Se elimina el div con clase btn-style-1 y se aplica la clase directamente al button -->
                  <button class="btn btn-primary btn-style-1" type="submit">Enviar</button>
                </div>
              </div>
            </div>
          </form>

        </div>
      </div>
    </div>
  </section>

  <!-- Thought part end  -->




  <!-- customers part start  -->



  <!-- customers part end  -->





  <!-- skills part start  -->




  <section class="skills">
    <div class="container">
      <div class="row ">
        <div class="col-lg-12">
          <div class="skill-text">
            <h2>Contamos <br>
              con el respaldo de.</h2>
          </div>

        </div>
      </div>
      <div class="row mar-top">

        <?php foreach ($respaldo as $item) : ?>
          <div class="col-lg-4  m-r">
            <div class="service-overelay"></div>
            <img class="bordered-image" style=" height: auto; display: block; margin: 0 auto;" src="<?= base_url('public/assets/image/others/' . $item['logo_respaldo']); ?>" alt="">
          </div>
        <?php endforeach; ?>
      </div>
    </div>
  </section>
  <!-- skills part end  -->
</main>

<script>
  // Obtén una referencia al formulario
  var formularioContacto = document.getElementById("formulario-contacto");
  var formMessages = document.getElementById("form-messages");
  var submitButton = formularioContacto.querySelector("button[type='submit']"); // Obtener el botón de envío


  formularioContacto.addEventListener("submit", function(event) {
    console.log("Form submit event triggered"); // LOG 1: Evento de submit disparado
    event.preventDefault(); // Previene el envío del formulario por defecto
    formMessages.innerHTML = ''; // Limpiar mensajes previos

    // Agregar la validación aquí
    var tipo_documento = document.getElementById("tipo_documento");
    var numero_documento = document.getElementById("numero_documento");
    var nombres = document.getElementById("nombres");
    var email = document.getElementById("email");
    var telefono = document.getElementById("telefono");
    var descripcion = document.getElementById("descripcion");

    console.log("Validating form data..."); // LOG 2: Iniciando validación
    let isValid = true;
    let errorMessages = [];

    if (numero_documento.value.trim() === "") {
        isValid = false;
        errorMessages.push('El número de documento es obligatorio.');
    }

    if (nombres.value.trim() === "") {
        isValid = false;
        errorMessages.push('El nombre es obligatorio.');
    }
    if (email.value.trim() === "") {
        isValid = false;
        errorMessages.push('El email es obligatorio.');
    } else {
        const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        if (!emailPattern.test(email.value.trim())) {
            isValid = false;
            errorMessages.push('El formato del email no es válido.');
        }
    }
    if (descripcion.value.trim() === "") {
        isValid = false;
        errorMessages.push('La descripción es obligatoria.');
    }

    if (!isValid) {
      console.log("Form is NOT valid. Errors:", errorMessages); // LOG 3a: Formulario no válido
      Swal.fire({
        icon: 'error',
        title: 'Campos Obligatorios o Inválidos',
        html: errorMessages.join('<br>')
      });
      return;
    }

    console.log("Form IS valid. Proceeding to send data via fetch."); // LOG 3b: Formulario válido, procediendo con fetch
    
    // Deshabilitar botón y mostrar carga
    submitButton.disabled = true;
    submitButton.innerHTML = '<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Enviando...';

    // Envía el formulario utilizando AJAX
    fetch(formularioContacto.action, {
        method: formularioContacto.method,
        body: new FormData(formularioContacto),
      })
      .then(function(response) {
        console.log("Fetch: Received response from server."); // LOG 4: Respuesta recibida del fetch
        return response.json(); 
      })
      .then(function(data) {
        console.log("Fetch: Data processed.", data); // LOG 5: Datos JSON procesados
        if (data.status === 'success') {
          Swal.fire({
            icon: 'success',
            title: '¡Enviado!',
            text: data.message,
          });
          formularioContacto.reset(); 
        } else if (data.status === 'error') {
          let messages = data.message;
          if (data.errors) {
            messages += '<br>' + Object.values(data.errors).join('<br>');
          }
          Swal.fire({
            icon: 'error',
            title: 'Error al enviar',
            html: messages,
          });
        }
      })
      .catch(function(error) {
        console.error("Fetch: Error occurred.", error); // LOG 6: Error en el fetch
        Swal.fire({
          icon: 'error',
          title: 'Error',
          text: 'Ocurrió un error al procesar la solicitud.',
        });
      })
      .finally(function() {
        // Volver a habilitar el botón y restaurar texto original
        submitButton.disabled = false;
        submitButton.innerHTML = 'Enviar';
      });
  });
</script>

<!-- footer part start  -->

<?php include('plantilla/footer.php')  ?>