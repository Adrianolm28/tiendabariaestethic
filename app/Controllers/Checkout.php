...
class Checkout extends BaseController
{
    ...
    public function ingresarDatos()
    {
        try {
            ...
            if (session()->has('usuario_autenticado')) {
                ...
                // Si no está autenticado, usar el usuario predeterminado
                $userData = $this->usuarioModel->find($usuarioPredeterminadoID);

                // Si no se encuentra el usuario predeterminado, redirigir con un error
                if (!$userData) {
                    return redirect()->to('/tienda/carrito')->with('error', 'Debes iniciar sesión para continuar.');
                }

                // Establecer los datos del usuario predeterminado en la sesión
    ...
    public function getDNI_New()
    {
        try {
            ...
            if ($dni) {
                $url = "https://apirest.valeapp.pe/reniec?dni=$dni";
    ...
    public function compraexitosa()
    {
        ...
        try {
            ...
            if ($formData && $idUsuario && $carrito) {
                ...
                if (!$compraExistente) {
                    ...
                    if ($idCompra) {
                        ...
                        foreach ($carritoSeguro as $producto) {
                            $precioFinal = ($producto['precio_transferencia'] > 0) ? $producto['precio_transferencia'] : $producto['precio'];
                            $detalleCompra = [
                                'id_compra' => $idCompra,
                                'id_producto' => $producto['id'],
                                'nombre' => $producto['nombre'],
            ...
            $comprasRecientes = session()->get('compras_recientes') ?? 0;

            // Incrementar el contador de compras recientes
            $comprasRecientes++;
        
            // Actualizar la sesión con el nuevo valor
            session()->set('compras_recientes', $comprasRecientes);
            /* fin */
   ...
   public function guardarCompra()
    {
        ...
        try {
            ...
            $sessionData = session()->get();
            $idUsuario = isset($sessionData['id_usuario']) ? strval($sessionData['id_usuario']) : '';
            $descuento = session()->get('descuento', 0);
            ...
            $ubicacionRecojo = $this->request->getPost('ubicacion_recojo');
            $fechaRecojo = $this->request->getPost('fecha_recojo') ?: null;
            $horaRecojo = $this->request->getPost('hora_recojo') ?: null;
            $nombreRecojo = $this->request->getPost('nombre_recojo') ?: null;
            $tipoEntrega = $this->request->getPost('tipo_entrega') ?? 'envio';

            if ($tipoEntrega === 'recojo') {
                $ubicacionRecojo = 'Calle Juana Manrique de Luna 168 - San Miguel';
            } else {
                $ubicacionRecojo = $direccion;
            }
            ...
            $nombreImagen = null;
            if ($imagenVoucher && $imagenVoucher->isValid() && !$imagenVoucher->hasMoved()) {
            ...
            foreach ($carrito as $producto) {
                ...
                $precioReal = isset($productoDB['precio']) ? floatval($productoDB['precio']) : 0;
                $precioTransferenciaReal = isset($productoDB['precio_transferencia']) ? floatval($productoDB['precio_transferencia']) : 0;
                $precioFinal = ($precioTransferenciaReal > 0) ? $precioTransferenciaReal : $precioReal;
                ...
                $carritoSeguro[] = [
                    ...
                    'cantidad' => $producto['cantidad'],
                    'precio' => $precioReal,
                    'precio_transferencia' => $precioTransferenciaReal
                ];
            }
            if ($descuento > 0) {
            ...
            if ($idCompra) {
                ...
                // Enviar correo electrónico with los detalles de la compra
                $email = \Config\Services::email();

                $mensaje = '<html><body>';
                $mensaje .= '<div style="text-align: center;"><img src="https://tegnex.pe/public/assets/image/others/logo/logotegnex.png" alt="" style="display: block; margin: 0 auto;"></div>';
                $mensaje .= '<h2 style="background-color: #144c9c; color: #fff; font-weight: 900; font-size: 24px; text-align: center">Detalles de tu compra</h2>';
                $mensaje .= '<p style="color: #144c9c; font-size: 14px; font-weight: 500;">Gracias por tu compra. Aquí están los detalles:</p>';
                $mensaje .= '<table style=" width: 100%; border-collapse: collapse;">';
                $mensaje .= '<tr style="background-color: #144c9c; color: #fff; border-radius: 10px;">
                                <th style="border-top-left-radius: 10px; border-bottom-left-radius: 10px; padding: 10px;">Producto</th>
                                <th style="padding: 10px;">Cantidad</th>
                                <th style="padding: 10px;">Precio</th>
                                <th style="border-top-right-radius: 10px; border-bottom-right-radius: 10px; padding: 10px;">Total por Producto</th>
                            </tr>';

                foreach ($compraDetalles as $detalle) {
                    $precioTotalProducto = $detalle['precio'] * $detalle['cantidad'];
                    $mensaje .= '<tr>';
                    $mensaje .= '<td style="text-align: center; font-weight: 500; color: #000;">' . $detalle['nombre'] . '</td>';
                    $mensaje .= '<td style="text-align: center; font-weight: 500; color: #000;">' . $detalle['cantidad'] . '</td>';
                    $mensaje .= '<td style="text-align: center; font-weight: 500; color: #000;">' . 'S/ ' . number_format($detalle['precio'], 2) . '</td>';
                    $mensaje .= '<td style="text-align: center; font-weight: 500; color: #000;">' . 'S/ ' . number_format($precioTotalProducto, 2) . '</td>';
                    $mensaje .= '</tr>';
                }
                $mensaje .= '<tr>';
                $mensaje .= '<td colspan="3">Costo de envío</td>';
